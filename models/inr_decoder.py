...
class INR_Decoder(nn.Module):
    ...
    def inference(self, coords, latent_vec, condition_vec, img_shape, tfs=None, step_size=100000):
        ...
        """
        if tfs is not None:
            print(f"tfs: {tfs.shape}, expand -> {tfs.expand(coords.shape[0], -1).shape}")
        ...
        if self.args['mask_reconstruction']:
            rec_masked = self.mask_reconstruction(modalities_rec, seg_hard)
            return rec_masked
        else:
    ...
    def connected_components(self, seg, bg_label_str='bg'):
        ...
        mask_blur = (ndi.gaussian_filter(mask.astype(np.float32), sigma=1.0) > 0.01).astype(np.uint8)
