...
class Data(Dataset):
    def __init__(self, args, tsv_file, split, df_loaded=None):
        ...
        self.weight_path = self.args['spatial_weighting']['path']
        self.w_obs = self.args['spatial_weighting']['w_obs']
        self.w_int = self.args['spatial_weighting']['w_int']
    def __len__(self):
        return len(self.df)
    
    def __getitem__(self, idx):
        row_dict = self.df.iloc[idx].to_dict()
        subj_id = row_dict.get("subject_id", f"idx_{idx}")
        mask_sample = self._load_mask_sample(self.weight_path,subj_id)
        mask_sample = np.asanyarray(mask_sample.dataobj).astype(np.uint8)
        modalities = self.load_modalities(row_dict)
        coords, values, sample_values, n_total, n_sample = self.load_coords_and_values(modalities,mask_sample=mask_sample)
        coords = torch.tensor(coords, dtype=torch.float32)
        values = torch.tensor(values, dtype=torch.float32)
        sample_values = torch.tensor(sample_values, dtype=torch.float32)

        if n_total == 0 or n_sample == 0:
            raise ValueError(f"n_total or n_sample==0 (no nonzero voxels in last_mod). subj={subj_id}, idx={idx}")
        else:
            w0 = float(self.w_int)
            w1 = float(self.w_obs)
            sample_weight = torch.where(
                sample_values > 0.5,
                torch.full_like(sample_values, w1),
                torch.full_like(sample_values, w0),
            )
        ...
        return coords, values, conditions, idx_df, sample_values, sample_weight
    
    def _parse_subj_id(self, subj_id: str):
        parts = str(subj_id).split("_")
        if len(parts) < 3:
            raise ValueError(f"Bad subject_id format: {subj_id}")
        return parts[1], parts[2]    
    
    def _load_mask_sample(self, path, subj_id: str):
        sid2, sid3 = self._parse_subj_id(subj_id)
        mask_path = os.path.join(path, f"{sid2}_{sid3}_mask_sample.nii.gz")
        if not os.path.exists(mask_path):
            raise FileNotFoundError(f"mask3 not found for {subj_id}: {mask_path}")
    ...
    def collate_fn(self, batch, shuffle=True):
        ...
        sample_values = torch.concat([b[4] for b in batch], dim=0)
        sample_weight = torch.concat([b[5] for b in batch], dim=0)
        if shuffle:
            ...
            idx_df = idx_df[perm]
            sample_values = sample_values[perm]
    ...
    def load_coords_and_values(self, modalities, mask_sample=None, normalize=True):
        ...
        sample_values = mask_sample[c_nz[:, 0], c_nz[:, 1], c_nz[:, 2]].flatten()
        n_total = c_nz.shape[0]
        n_sample = int(np.sum(sample_values == 1))
        c_nz = nib.affines.apply_affine(affine, c_nz)
        coords = c_nz # center the image in voxel space
        if normalize:
        ...
        return coords, values, sample_values, n_total, n_sample
