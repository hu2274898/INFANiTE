...
class Criterion(nn.Module):
    def __init__(self, args):
        ...
        self.criterion_sr = nn.MSELoss(reduction='none') if args['optimizer']['loss_metric'] == 'mse' else nn.L1Loss(reduction='none')
        ...
        self.criterion_seg = nn.CrossEntropyLoss(weight=self.ce_weights, reduction='none')

    def forward(self, output, target, tfs, sample_values, sample_weights, sr_weight=1.0, seg_weight=1.0):
        sr_raw = self.criterion_sr(output[..., :self.sr_dims], target[..., :self.sr_dims]) 
        sr_point = sr_raw.mean(dim=-1)
        w = sample_weights.view(-1).to(sr_point.dtype)
        
        loss = {'seg': torch.tensor(0.0), 
                'sr': sr_weight * (sr_point * w).sum() / (w.sum() + 1e-8),
                'trafo': torch.tensor(0.0), 
                'total': 0.0}

        if seg_weight > 0:
            seg_raw = self.criterion_seg(output[..., self.sr_dims:], target[..., -1].to(torch.int64))  # (N,)
            w = sample_weights.view(-1).to(seg_raw.dtype)  # (N,)
            loss['seg'] = seg_weight * (seg_raw * w).sum() / (w.sum() + 1e-8)
...
def add_background_halo(label_names, seg_nii, halo_width=1.5, background_label_str='bg'):
    ...
    mask_bg = (ndi.gaussian_filter(mask_bg, sigma=halo_width) > 0.01).astype(np.uint8) * bg_label
